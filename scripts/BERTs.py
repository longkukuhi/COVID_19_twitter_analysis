...
class Bertnn:
    def __init__(self, train_inputs, train_labels, train_masks, validation_inputs, validation_labels, validation_masks,
                 test_batch, batch_size=16, epochs=4, lr=None, category_number=2, categories=None,
                 evluate_methods=None):
        ...
        if categories == None:
            self.categories = ['GoodsServices', 'InformationWanted', 'Volunteer', 'MovePeople', 'EmergingThreats',
                               'NewSubEvent', 'ServiceAvailable', 'Advice', 'Any']
        else:
    ...
    def searchUpsample(self, increaseTimes, lr=None):
        ...
        for increaseTime in range(0, increaseTimes, 10):
            ...
            for category in range(self.category_number):
                ...
                train_inputs_upsample, train_labels_upsample, train_masks_upsample = Up_sample_Bertnn.upSample(
                    train_inputs_upsample,
...
class Up_sample_Bertnn(Up_sample):
    @classmethod
    def upSample(cls, trainSet, trainL, train_masks, category, increaseTimes):
...
class Evaluate_model_nn(Evaluate_model):
        ...
        for epoch_i in range(0, epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                for k in range(category_number):
                    tmp_train_precision[k], tmp_train_recall[k], tmp_train_f1[k] = Scoring_nn.flat_metrics(self, logits,
                                                                                                           label_ids, k)
            ...
            print("  Training epcoh took: {:}".format(Scoring_nn.format_time(time.time() - t0)))
            ...
            for batch in validation_dataloader:
                ...
                for k in range(category_number):
                    tmp_eval_precision[k], tmp_eval_recall[k], tmp_eval_f1[k] = Scoring_nn.flat_metrics(self, logits,
                                                                                                        label_ids, k)
            ...
            print("The average recall is: {0:.4f}".format(recall_avg / category_number))
            print("The average f1 is: {0:.4f}".format(f1_avg / category_number))
            print("  Validation took: {:}".format(Scoring_nn.format_time(time.time() - t0)))
    ...
    def evalTest(self, model, test_dataloader, category_number):
        ...
        for batch in test_dataloader:
            ...
            for k in range(category_number):
                tmp_test_precision[k], tmp_test_recall[k], tmp_test_f1[k] = Scoring_nn.flat_metrics(self, logits,
                                                                                                    label_ids, k)
...
class Evaluate_model_nn_rectified_binary(Evaluate_model_nn):
    def evalTrain(self, model, train_dataloader, validation_dataloader, epochs,
        ...
        for epoch_i in range(0, epochs):
            ...
            if avg_val_f1 > current_best_F1_avg:
                print("Found new best average F1 score model when validation, old f1 is %f , new f1 is %f" % (
                best_F1_avg, avg_val_f1))
    ...
    def evalTest(self, test_dataloader, category_number):
        '''This method is used to only validate trained BERT model on a test set'''
        # ========================================
        #               Testing
        # ========================================
    ...
    def searchUpsample(self, increaseTimes, lr=None):
        ...
        for increaseTime in range(0, increaseTimes, 10):
            ...
            if self.lr != None:
                ...
                self.updateParameters(train_dataloader)
            # Use over-sample dataset to train and validate BERT model
            all_metrics[increaseTime], val_F1_avg = self.evluate_methods.evalTrain(self.model, train_dataloader,
                                                                                   self.validation_dataloader,
                                                                                   self.epochs,
                                                                                   self.category_number, self.optimizer,
                                                                                   self.scheduler,
                                                                                   self.optimizer_small,
                                                                                   self.scheduler_small,
...
class Bertnn_rectified_priorization(Bertnn):
    ...
    def searchUpsample(self, increaseTimes, lr=None):
        ...
        for increaseTime in range(0, increaseTimes, 10):
            ...
            for category in range(self.category_number):
                ...
                print("Current is processing category %d" % (category))
                # build the up-sample data set
                train_inputs_upsample, train_labels_upsample, train_masks_upsample = Up_sample_Bertnn.upSample(self,
                                                                                                               train_inputs_upsample,
                                                                                                               train_labels_upsample,
                                                                                                               train_masks_upsample,
                                                                                                               category,
            ...
            all_metrics[increaseTime], val_F1_avg = self.evluate_methods.evalTrain(self.model, train_dataloader,
                                                                                   ...
                                                                                   self.scheduler,
                                                                                   self.optimizer_small,
                                                                                   self.optimizer_medium,
                                                                                   self.scheduler_small,
                                                                                   self.scheduler_medium,
                                                                                   best_F1_avg=best_F1_avg)
            # Store return metrics result
    ...
    def upSampleTrain(self, increaseTime):
        ...
        return self.evluate_methods.evalTrain(self.model, train_dataloader, self.validation_dataloader, self.epochs,
                                              self.category_number, self.optimizer, self.scheduler,
                                              self.optimizer_small, self.optimizer_medium, self.scheduler_small,
                                              self.scheduler_medium)
