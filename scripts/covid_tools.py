...
# Version: 2.00 Update date: 20/09/2020
################
...
class Up_sample():
  ...
  def up_sample(cls, location, category, increaseTime, testF, tokenizer):
    '''
    This function build up-sample dataset
  ...
  def build_upsample_set(cls, trainSet, trainL_single, location, category, increaseTime):
    ...
    # print('After length %d'%(len(label_temp)))
    # print(len(trainSet_temp['hashtags']))
    # print(len(trainSet_temp['full_text']))
    # print(len(trainSet_temp['favorite_count']))
    # print(len(label_temp))
...
class Evaluate_model():
  ...
  @classmethod
  def cross_validation_result_multilabels(cls, cf, dataset, labels):
    '''
    This function run cross validation on given dataset and return average score
    :param cf: classifier to evaluate
    :param dataset: dataset used to eavluate
  ...
  def total_evaluation(cls, model, model_name, runningSet,
                       print_out_detail=False, multi_sign=False
                       ):
    '''
    ...
    # Cross validation
    if multi_sign == True:
      cv_scores = Evaluate_model.cross_validation_result_multilabels(model, train_set, train_labels)
    else:
    ...
    print('Testing result on test set:')
    model.fit(train_set, train_labels)
    test_predicted_labels = model.predict(test_set)
    precision_test, recall_test, f1_test = Evaluate_model.evaluation_summary(None, model_name + "result on test set",
                                                                     test_predicted_labels, test_labels)
    print('Cross validation: Average validation set result: %s=%0.6f %s=%0.6f %s=%0.6f.'
          % (precision_test,
             recall_test,
             f1_test,
             ))
  ...
  @classmethod
  def get_errors(cls, predictions, data):
    '''
  ...
  @classmethod
  def get_errors_indexs(cls, predictions, data):
    '''
...
class Process_labels():
  ...
  def extract_majority_vote_label(self, frame):
    '''
...
class Classical_classifiers():
  ...
  def build_best_lr_priority(self, cf, vect_count, category, increaseTime, trainF, trainL, testF, testL):
    ...
    Classical_calssifiers.lr_priority(runningSet, cf)
  ...
  def build_default_lr_priority(self, location, category, increaseTime, trainF, trainL, testF, testL, tokenizer,
    ...
    print("Display result without up-sampleing: ")
    runningSet = [trainF_union, trainL, testF_union, testL]
    Classical_calssifiers.lr_priority(runningSet)
  ...
  def DecisionTreeCf(self, runningSet):
    '''
    ...
    # unpack running set
    train_features, train_labels = runningSet[:2]
    testF_union_tmp = runningSet[2]
    testL = runningSet[3]
    decisionTreeCf = DecisionTreeClassifier()
    # use evalaution method to evalute the performance of classifier
    cv_scores, decisionTreeCf = Evaluate_model.total_evaluation(None, decisionTreeCf, 'DecisionTree', runningSet, multi_sign=True)
    # Plot decision tree
    ...
    # evaluate on the test set
    decisionTreeCf.fit(train_features, train_labels)
    predictions = decisionTreeCf.predict(testF_union_tmp)
    precision = precision_score(predictions, testL, average='macro')
    recall = recall_score(predictions, testL, average='macro')
    accuracy = accuracy_score(predictions, testL)
    f1 = fbeta_score(predictions, testL, 1, average='macro')
  ...
  def svm_priority(runningSet, print_out_detail=False):
    '''
    ...
    svm_cf.fit(train_set, train_labels)
    # choose metrics based on different number of categories
    type_number = len(set(testL))
    predictions = svm_cf.predict(testF)
    if type_number <= 2:
      precision = precision_score(predictions, testL)
      recall = recall_score(predictions, testL)
      accuracy = accuracy_score(predictions, testL)
...
'''
############################################################
Individual function (Basically same as encasulate classes)
############################################################
'''
################
# Functions and class coded by Zijun Long for classifying Covid-19 related tweets
...
################### B
def build_best_lr_priority(cf, vect_count, category, increaseTime, trainF, trainL, testF, testL):
  '''
  This function build best logistic regression model for priorization task
  :param cf: classifier to use
  :param vect_count: Count Vecoterizer
...
def build_default_lr_priority(location, category, increaseTime, trainF, trainL, testF, testL,
                              tokenizer, ):
  '''
    This function build  logistic regression model with default setting for priorization task
    :param location: indicate location of current data set
  ...
  # Step 2: featureunion the up-sample data set
  fu_best = FeatureUnion([('full_text', myPipeline([('selector', ItemSelector(key='full_text')), ('cv', vect_count),
                                                    ])),
                          ('hashtags', myPipeline([('selector', ItemSelector(key='hashtags')),
                                                   ('cv', vect_count),
                                                   ])),
...
def build_upsample_set(trainSet, trainL_single, location, category, increaseTime):
  '''
    This function use over-sample method to build over-sampling
    data set and store them into csv file
    :param trainSet: training features
...
def build_best_lr(location, cf, vect_count, vect_tfidf, category, increaseTime,
                  trainF, trainL_single, testF, testL_single):
  '''
  This function build the best logistic regression model based
  on the best hyper-parameters and evaluate it
  ...
  # train the classifiers and evaluate the result
  runningSet = [trainF_up_one_union, trainL_up_one, testF_union, testL_single[category]]
  evaluate_one_cf(cf, runningSet, category)
...
def build_best_lr_without_featureunion(location, cf, vect_tfidf, category, increaseTime,
                                       trainF, trainL_single, testF, testL_single):
  '''
  This function build logistic regression model through only use full-text feature
  :param location: indicate location of current data set
...
def build_all_labels():
  '''
    This function read labelled tweets from five json files
    and merge them into one dataFrame
    :return:
    '''
  import json
...
def build_union_labels(all_labels):
  '''
  This function find all labels(lines in dataFrame) for one tweet
  and union all labels, then store them into a new dataFrame without duplication
  :param all_labels: A dataFrame contains all labelled tweets
...
def build_default_lr(location, category, increaseTime, trainF, trainL_single,
                     testF, testL_single, tokenizer):
  '''
...
def cross_validation_result(cf, dataset, labels):
  '''
  This function run cross validation on given dataset and return average score
  :param cf: classifier to evaluate
  :param dataset: dataset used to eavluate
...
def cross_validation_result_multilabels(cf, dataset, labels):
  # Cross validation
...
################### D
def display_result_without_upsample(cf, category, runningSet):
  '''
  This function print out metrcis of classifier without appling over-sampling
  :param cf: classifier to evaluate
  :param category: current processing category
  ...
  cf.fit(train_set, train_labels)
  # use evalaution method to evalute the performance of classifier
  total_evaluation(cf, model_name, runningSet)
...
def DummyResult(runningSet):
  ...
  dummy_cf = DummyClassifier(strategy='most_frequent')
  dummy_cf.fit(train_features, train_labels)
  cv_scores = total_evaluation(dummy_cf, 'Dummy', runningSet, multi_sign=True)
  # record_result('dummy_most_train',precision_train,recall_train,f1_train)
  # record_result('dummy_most_test',precision_test,recall_test,f1_test)
...
def DecisionTreeCf(runningSet):
  ...
  decisionTreeCf = DecisionTreeClassifier()
  # use evalaution method to evalute the performance of classifier
  cv_scores = total_evaluation(decisionTreeCf, 'DecisionTree', runningSet, multi_sign=True)
  # Plot decision tree
  from sklearn.tree import plot_tree
  plt.figure()
  plot_tree(decisionTreeCf)
...
def extractLabels_legacy(data, labels):
  '''
  old version of TREC-IS COVID-19 task, no longer use
  '''
  categories = ['GoodsServices', 'InformationWanted', 'Volunteer', 'MovePeople', 'EmergingThreats', 'NewSubEvent',
                'ServiceAvailable', 'Advice', 'Any']
  labels_all = []
...
def extract_one_feature(dataFrame, featureName):
  '''
  Extract one feature from dataFrame
  :param dataFrame: dataFrame contains all information of all tweets
  :param featureName: name of one feature
...
def extract_features(dataFrame, featureList):
  '''
  Extract mulitple features by using extract_one_feature function
  :param dataFrame: dataFrame contains all information of all tweets
  :param featureList: a list contains names of wanted features
  ...
  all_ft = {}
  for i in range(len(featureList)):
    all_ft[featureList[i]] = extract_one_feature(dataFrame, featureList[i])
  return all_ft
...
def extract_hashtags(dataset):
  ...
  '''
  dataset_after = dataset.copy()
  dataset_after['hashtags'] = []
  for i in range(len(dataset['entities'])):
    hashtags = ' '
    for j in range(len(dataset['entities'][i]['hashtags'])):
      hashtags += (dataset['entities'][i]['hashtags'][j]['text']) + ' '
    dataset_after['hashtags'].append(hashtags)
  return dataset_after
...
def evaluate_one_cf(cf, runningSet, category):
  '''
  This function evaluate the given classifier
  :param cf: classifier to evaluate
  :param runningSet: A list contrains training features, training labels
  ...
  # choose metrics based on different number of categories
  if type_number <= 2:
    cv_scores = total_evaluation(lr, 'LogisticRegression', runningSet)
...
def extract_majority_vote_label(frame):
  '''
  This function extract duplicate priority labels by majority vote method
  More specifically,if one tweet is assessed by more than two assessors,
  we use the most common label as thepriority label.
...
################### G
def get_errors(labels, predictions, data):
  '''
  This function build a new dataFrame which contains all information of errors
...
def get_errors_indexs(labels, predictions, data):
  '''
  This function return indexs of error
  :param labels: true labels
  :param predictions: prediction of classifier
...
def generateTrainingSetCV(frame):
  '''
  This function split whole dataset into training set and
   test set and the ratio is 0.3
  :param frame: whole dataset in dataFrame
...
################### L
def lr_Binary(runningSet, category, categories):
  '''
  This function build and evaluate a binary logistic regression function
  :param runningSet: A list contrains training features, training labels
                        test features, test labels
...
def lr_priority(runningSet, print_out_detail=False, cf=None):
  '''
  This function build and evaluate a  logistic regression function
  for prioritization task
  :param runningSet: A list contrains training features, training labels
  ...
  if cf == None:
    ...
    lr = cf
  # lr.fit(train_set,train_labels)
  cv_scores = total_evaluation(lr, 'LogisticRegression', runningSet, print_out_detail)
...
def plot_pie_priority_count(frame, location):
  '''
  This function plot pie figure for priority labels
  :param frame:  whole dataset in dataFrame
  :param location: current processing location
...
def print_all_featrues_of_one_sample(sample, idx):
  # print all detail of one tweet in dataFrame
...
def print_raw_errors(labels, predictions, data):
  # print all detial of errors
  label_arr = labels
  previous_text = None
  for idx, prediction in enumerate(predictions):
    label = label_arr[idx]
    if prediction != label and data['full_text'][idx] != previous_text:
      previous_text = data['full_text'][idx]
      print(idx, label, prediction, data['full_text'][idx], data['hashtags'][idx])


################### Q

################### R
def read_covid_dataset_mac():
  ...
  return cv19_dc, cv19_ny, cv19_ws


def read_union_labels_from_file():
  all_labels = pd.read_csv('cache/all_labeled.csv',
                           converters={"priority": lambda x: x.strip("[]").replace("'", "").split(", "),
                                       "categories": lambda x: x.strip("[]").replace("'", "").split(", ")})
  return all_labels


def record_nine_result(train_record, val_record, test_record, scores, test_scores, category):
  train_record[category]['accuracy'].append(list(scores.values())[3])
  train_record[category]['precision'].append(list(scores.values())[5])
  train_record[category]['f1'].append(list(scores.values())[7])
  train_record[category]['recall'].append(list(scores.values())[9])

  val_record[category]['accuracy'].append(list(scores.values())[2])
  val_record[category]['precision'].append(list(scores.values())[4])
  val_record[category]['f1'].append(list(scores.values())[6])
  val_record[category]['recall'].append(list(scores.values())[8])

  test_record[category]['accuracy'].append(test_scores['accuracy'])
  test_record[category]['precision'].append(test_scores['precision'])
  test_record[category]['f1'].append(test_scores['f1'])
  test_record[category]['recall'].append(test_scores['recall'])


################### S
def showCountingOfPredicted(df, className):
  oneClass = df[df['Predicted'] == className]
  print(oneClass.groupby('True labels').count())


def showCountingOfTrueLabels(df, className):
  oneClass = df[df['True_labels'] == className]
  # print("Classifier result for %s" %(className))
  # print(oneClass)
  # print()
  print('Statis result for %s' % (className))
  print(oneClass.groupby('Predicted').count())
  print()


def search_best_lR(increaseTimes, tokenizer, testF, testL):
  '''
  This function search the best over-sample factor for classifier
  :param increaseTimes: over-sample factor
  :param tokenizer: tonizer to tokenize
...
def svm_priority(runningSet, print_out_detail=False):
  ...
  svm_cf = SVC()
  # lr.fit(train_set,train_labels)
  cv_scores = total_evaluation(svm_cf, 'SVM', runningSet, print_out_detail)
...
def store_train_test_split_to_csv(df, location, target, val_ratio=0.2):
  # split dataset and store into csv file
  idx = np.arange(df.shape[0])
...
def total_evaluation(model, model_name, runningSet, print_out_detail=False, multi_sign=False
                     ):
  '''
  Conduct total evaluation on given dataset and model
  :param model: model to evalutae
  ...
  # Cross validation
  if multi_sign == True:
    cv_scores = cross_validation_result_multilabels(model, train_set, train_labels)
  else:
    cv_scores = cross_validation_result(model, train_set, train_labels)
  ...
  if print_out_detail == True:
    ...
    for train_index, test_index in kf.split(train_set):
      ...
      precision_train, recall_train, f1_train = evaluation_summary(model_name + "result on training set",
                                                                   train_predicted_labels, y_train)

      precision_test, recall_test, f1_test = evaluation_summary(model_name + "result on validation set",
                                                                test_predicted_labels, y_test)

  return cv_scores


def types_statistics(frame, location):
  '''
  Print out some statistics of given location
  :param frame: whoel data set
  :param location: name of processing location
  ...
  print(location + ":")
  print(frame.categories.value_counts())
  plot_pie_priority_count(frame, location)
...
################### U
def up_sample(location, category, increaseTime, testF, tokenizer):
  '''
  This function build up-sample dataset
  :param location: name of processing location
  :param category: indicate current processing category
  ...
  df_tmp = pd.read_csv('cache/' + location + '/upsampleData_' + str(category) + '_' + str(increaseTime) + '.csv')
  featureList = ['full_text', 'hashtags', 'favorite_count']
  trainSet_temp = extract_features(df_tmp, featureList)
  labelList = ['target']
  train_label_temp = extract_features(df_tmp, labelList)['target']
